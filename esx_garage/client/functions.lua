...
ImpoundClosestVehicle = function()
    ...
    if DoesEntityExist(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) <= 5.0 then
        ...
        ESX.TriggerServerCallback("playooze_garage:impound", function(res)
            if res then
...
Marker = function(position, text, control, distancecheck)
    DrawMarker(-1, position - vec3(0.0, 0.0, 1.0), vec3(0.0, 0.0, 0.0), vec3(0.0, 0.0, 0.0), vec3(2.0, 2.0, 0.2), 100, 100, 100, 27, false, true) --distancecheck = 15.0  --- aparecer blip
    if not distancecheck then distancecheck = 5.0 end
    if #(GetEntityCoords(PlayerPedId()) - position) <= distancecheck and text then
        DrawTxt3D(position, text, true)
        if IsControlJustReleased(0, control or 304) then
            return true
        end
    end
end
...
Citizen.CreateThread(function()
    ...
    local text = Strings["garage_interior"]
    while true do
        wait = 5
        local ped = PlayerPedId()
        local inZone = false
        local dist = #(GetEntityCoords(ped)-vector3(229.52, -990.52, -99.0))
        if dist <= 20.0 then
            wait = 5
            inZone  = true
